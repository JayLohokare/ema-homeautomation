...
public class MainActivity extends AppCompatActivity
    ...
    private ListAdapterRoom adapter;
    private ListView listView;
    public RowListHandlerRoom rowListHandler;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rowListHandler = new RowListHandlerRoom();
        rowListHandler.clearAll();
        adapter = new ListAdapterRoom(this, rowListHandler);
    ...
    public void handleResult(JsonObject result, String action) throws JSONException {
        ...
        Log.i("Json response", rooms);
        List<String> roomlist = Arrays.asList(rooms.split(","));

        for (int i = 0; i < roomlist.size(); i++) {
            if(!roomlist.get(i).equals("") && !roomlist.get(i).equals("android")){
                rowListHandler.addRow(new RowObjectRoom(roomlist.get(i)));
                adapter.notifyDataSetChanged();
                Log.i("row list", String.valueOf(rowListHandler.getListsize()));
            }
        }
    }
}
